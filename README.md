...
- a section with the Preview section (if `preview` is enabled, otherwise this section does not appear). The Review and Editor sections are toggled.

Above the editor section (generated by the simplemde Jquery plugin), there are two buttons **SAVE** and **PREVIEW**. When **SAVE** is clicked, the edited **md** content is transferred back to the `*.md` file associated with the `route`, and an entry is made in the log file giving the date, the user name, the route edited and a record of the change.
...
Note that only changes that have been **Saved** will be visible in the **Review** section.
...
Note that the access permission is the one granted in the `reviewers` group above. For this example, we gave *peter* `reveiew` permission. The page generates the following
